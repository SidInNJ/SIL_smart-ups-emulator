...
//extern const DeviceDescriptor USB_DeviceDescriptorIAD PROGMEM;     // DBC.008
extern DeviceDescriptor USB_DeviceDescriptorIAD;                     // DBC.008
...
{                                                                                  // DBC.008a
	...
	USB_DeviceDescriptorIAD.dtype = 1;                                               // DBC.008a
  ...
  //if (USBCDCNeeded) {                                  // DBC.008d Switch is pressed                 // Davis - Device descriptor is wrong for CDC_DISABLED
      ...
      if (!USBCDCNeeded)                                                        // SLR 2024-04-19
      {
		...
		//USB_DeviceDescriptorIAD.deviceProtocol = 0x00;                                 // DBC.008a
		USB_DeviceDescriptorIAD.deviceClass = 0xEF;                                    // DBC.008b
		USB_DeviceDescriptorIAD.deviceSubClass = 0x02;                                 // DBC.008b
		USB_DeviceDescriptorIAD.deviceProtocol = 0x01;                                 // DBC.008b
		USBSwitchTime[0] = millis();                                                   // DBC.008c
		USBSwitchCount[0]++;                                                           // DBC.008d
	}                                                                                // DBC.008a
	else {                                                // Serial port enabled     // DBC.008a
	...
	USB_DeviceDescriptorIAD.iSerialNumber = ISERIAL;                                 // DBC.008a
...
bool SendConfiguration(int maxlen)
{
	...
	ConfigDescriptor config = D_CONFIG(_cmark + sizeof(ConfigDescriptor),interfaces);  // Davis - number of interfaces is wrong for CDC_DISABLED
...
bool SendDescriptor(USBSetup& setup)
{
	...
	u8 desc_length;                                  // DBC.008
	//u8 desc_length = pgm_read_byte(desc_addr);     // DBC.008
	if (progMem)                                     // DBC.008
	{                                                // DBC.008
		desc_length = pgm_read_byte(desc_addr);        // DBC.008
		USB_SendControl(TRANSFER_PGM,desc_addr,desc_length);  // DBC.008
	}                                                // DBC.008
	else
	{                                                // DBC.008
		desc_length = USB_DEVICE_DESC_SIZE;            // DBC.008   Davis, fix this. Get length from descriptor
		USB_SendControl(0,desc_addr,desc_length);      // DBC.008
	}                                                // DBC.008
